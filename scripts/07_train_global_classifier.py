#!/usr/bin/env python
"""Train optional V2 global ResNet18 multi-label classifier."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train global ResNet18 classifier.")
...
def main() -> int:
    ...
    if not bool(cfg.get("enabled", False)):
        logger.info("Global classifier disabled; skipping.")
        return 0
    ...
    train_dataset = GlobalCXRDataset(train_df, train_ids, image_index, columns, int(cfg["image_size"]))
    val_dataset = GlobalCXRDataset(train_df, val_ids, image_index, columns, int(cfg["image_size"]))
    ...
    if bool(cfg.get("resume", True)) and ckpt_path.exists():
        logger.info("Resuming global classifier from %s", ckpt_path)
    ...
    for epoch in range(epochs):
        train_loss = train_multilabel_epoch(model, train_loader, optimizer, device, amp=amp_enabled, scaler=scaler)
        val_loss = evaluate_multilabel_loss(model, val_loader, device) if len(val_dataset) else train_loss
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss})
        logger.info("Global epoch %d | train %.5f | val %.5f | best %.5f", epoch, train_loss, val_loss, best_loss)
        if val_loss < best_loss:
            best_loss = val_loss
            save_checkpoint(ckpt_path, model, optimizer, scaler, epoch, best_loss, config)

    summary_path = output_path(config, "lgcxr_global_classifier_training_summary.json")
    summary_path.write_text(json.dumps({"best_loss": best_loss, "history": history}, indent=2), encoding="utf-8")
    logger.info("Global classifier checkpoint: %s", ckpt_path)
    return 0
